...
#if defined(NDEBUG)
# error "Ragcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x6edfe2832bcd073500f9792893a9859c5bf32e71bbc2cadfe98cf0bce706bee9");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Ragcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Ragcoin Signed Message:\n";
...
{
    // Ragcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 259200); // Ragcoin: 259200 blocks in ~3 months

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Ragcoin: 1 day
static const int64 nTargetSpacing = 0.5 * 60; // Ragcoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Ragcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Ragcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x011fc4734f73fc7121962a7f22ab263117f70bea679efae93a6fac318f58319b");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This week’s top stories: iOS 11.4 beta";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 110000 * COIN; //1% premine for dev and marketing efforts
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("040605d58653e7edf6c1bbd46f0e28c13fcff2ddf0a764567787ae53a1fe1f7d595c0369fca51d3e50efc1fcce43a4420597a3855b47588419d435066abf129101") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1523138602;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085447181;

        if (fTestNet)
        {
            block.nTime    = 1523138588;
            block.nNonce   = 385543030;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xd02903c03a67fbabcd3ff020f25fcd34832212c63493168073b14e6caeac3858"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf0, 0xcc, 0xb8, 0xd3 }; // Ragcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RagcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RagcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static RagcoinMiner(CWallet *pwallet)
{
    printf("RagcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ragcoin-miner");
    ...
    try { loop {
        ...
        printf("Running RagcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("RagcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RagcoinMiner, pwallet));
}
